...
using Onderneming = Moza.Omc.Api.Models.Onderneming;
    ...
    IMapper mapper)
{
    ...
    {
        var notificatie = this.CreateInitialNotificatie(kvkNummer, reference, notificatieCreateRequest);
    ...
    private async Task<DeliveryStatuses> RandomStatus(string reference)
    {
        ...
        var returnStatus = DeliveryStatuses.Unknown;

        if (roll < 75)
        {
            //TODO [TB] Dit zou eingelijk via het callback endpoint moeten komen maar die gebruiken we nog niet.
            await notificatieEventRepository.CreateAsync(reference,
                new NotificatieEvent("Successvol antwoord ontvangen van NotifyNL", DeliveryStatuses.Created));
        ...
        {
            DeliveryStatuses.Cancelled, DeliveryStatuses.PermanentFailure, DeliveryStatuses.TemporaryFailure,
    ...
    private async Task VerstuurNotificatie(Notificatie? notificatie)
    {
        ...
        foreach (var contactMethode in notificatie.ContactMethodes)
        {
            ...
            if (contactMethode.Type != NotificationTypes.Brief)
            {
                ...
                {
                    contactValue = await this.GetEmailFromProfielService(contactValue, notificatie.Reference!);
                    contactType = NotificationTypes.Email;
                    notificatie.Type = contactType;
                }

                await this.CallNotifyNL(contactValue, contactType, notificatie);
                notificatie.Status = await this.RandomStatus(notificatie.Reference!);
            ...
            {
                notificatie = await this.VerstuurBrief(notificatie, contactValue);
    ...
    {
        await notificatieEventRepository.CreateAsync(notificatie.Reference!,
            new NotificatieEvent($"Notificatie verstuurd naar Logius Brieven Dienst voor adres {contactValue}", DeliveryStatuses.Sent));
    ...
    private async Task CallNotifyNL(string contactValue, NotificationTypes contactType, Notificatie notificatie)
    {
        ...
        await notificatieEventRepository.CreateAsync(notificatie.Reference!,
            new NotificatieEvent($"Notificatie verstuurd naar NotifyNL via {contactType} ({contactValue})", deliveryStatus));
    ...
    private async Task<string> GetEmailFromProfielService(string kvkNummer, string reference)
    {
        ...
        await notificatieEventRepository.CreateAsync(reference,
            new NotificatieEvent($"Email opgehaald bij de profiel service voor {kvkNummer} ({profielResponse.Onderneming.Email})",
    ...
    public async Task<Onderneming> UpdateNotificatie(Notificatie notificatie)
    {
        ...
        {
            KvkNummer = updatedNotificatie.KvkNummer, Notificaties = [mapper.Map<Notificatie>(updatedNotificatie)]
        };
